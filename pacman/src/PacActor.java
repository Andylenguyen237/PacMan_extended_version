...
public class PacActor extends Actor implements GGKeyRepeatListener
{
  ...
  private void eatPill(Location location)
  {
    ...
    if (c.equals(Color.white))
    {
      ...
      for (GoldPieces piece: this.game.getGoldPieces()){
        if (piece.getLocation().equals(location)){
          piece.effectOnPacActor(this);
          if(!this.game.getMonsterList().get(0).getStopMoving())
            piece.effectOnMonsters(this.game.getMonsterList(), this.game.getGameVersion());
        }
      }
      ...
      for (IceCubes ice: this.game.getIceCubes()){
        if (ice.getLocation().equals(location)){
          ice.effectOnMonsters(this.game.getMonsterList(), this.game.getGameVersion());
        }
      }
  ...
  public boolean hitEnemies(){
    for (Monster monster: this.game.getMonsterList()){
      if (monster.getLocation().equals(getLocation())){
        return true;
      }
    }
    return false;
